...
const ShopElement: NextPage<Props> = ({key,category,name,price,discount , image} : Props) => {
  ...
  return (
    <>
        <div key={key} className="card overflow-hidden rounded-none w-full h-full bg-base-100 shadow-xl cursor-pointer transition duration-300 ease-in-out hover:scale-[1.02]">
        <figure className="max-h-[200px] bg-white"><img className="h-[120px] sm:h-[200px]" src={image} alt="Shoes" /></figure>
            <div className="card-body p-0 pt-2  max-w-full  ">
                ...
                <div className=" w-full mt-auto px-4 ">
                    ...
                    <p className="text-xs lg:text-sm sm:inline  mr-3 text-secondary font-semibold">{(price*150*discount).toFixed(2)} dzd</p>
                    {discount != 1 ? <p className="text-xs lg:text-sm sm:inline  text-gray-400 line-through">{(price*150).toFixed(2)} dzd</p>:<></>}
                </div>
